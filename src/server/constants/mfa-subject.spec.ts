...
describe('mfaSubject', () => {
  ...
  it('scopes a dashboard subject by tenant and user', () => {
    expect(mfaSubject('dashboard', 'user-1', 'tenant-1')).toBe('dashboard:tenant-1:user-1')
  })
  ...
  it('derives a platform subject from the user alone', () => {
    expect(mfaSubject('platform', 'admin-1', undefined)).toBe('platform:admin-1')
  })
  ...
  it('ignores a tenant supplied on the platform plane', () => {
    expect(mfaSubject('platform', 'admin-1', 'tenant-1')).toBe('platform:admin-1')
    expect(mfaSubject('platform', 'admin-1', 'tenant-1')).toBe(
      mfaSubject('platform', 'admin-1', undefined)
    )
  })
  ...
  it('gives two tenants distinct subjects for the same user id', () => {
    expect(mfaSubject('dashboard', 'user-1', 'tenant-a')).not.toBe(
      mfaSubject('dashboard', 'user-1', 'tenant-b')
    )
  })
  ...
  it('never collides a dashboard subject with a platform subject', () => {
    expect(mfaSubject('dashboard', 'shared-id', 'tenant-1')).not.toBe(
      mfaSubject('platform', 'shared-id', undefined)
    )
  })
})
