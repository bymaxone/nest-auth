...
const ARTIFACT_COMMENT = [
  ...
  'schema derived faithfully would document them wrong:',
  ...
  '    exclusivity, so `required` cannot say it.',
  ...
  '    conditional requirement, so it appears optional here.',
  '',
  'Normalisation is invisible too: email fields are trimmed and lowercased before the service',
  'sees them, which no keyword expresses and every client should know.'
]
