...
const ARTIFACT_COMMENT = [
  ...
  '  - OAuthCallbackQueryDto.code is optional here AND in the DTO, and the two agree for once:',
  '    a callback carrying neither code nor error is refused by the handler as',
  '    auth.oauth_failed rather than by the pipe as auth.validation, because a codeless callback',
  '    is a failed authorization and not a malformed request. The overlay declares that refusal,',
  '    with probes both layers answer.',
  '',
