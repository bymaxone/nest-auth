...
| `jwt.absoluteSessionLifetimeDays` | `number`                         | No          | `0` (off)                             | Caps how long one login can be extended by rotation; `0` disables the cap                                                                                                                                           |
...
| ----------- | -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rt`        | `auth:rt:{sha256(token)}`                                | JSON with session data: `{ userId, tenantId, role, device, ip, createdAt, mfaEnabled, familyId }` (`familyId` omitted when empty) | `refreshExpiresInDays` (in days, converted to seconds)    | Refresh token session. Stores the data needed to reissue the access token without querying the database.                                                                                                                                                                                            |
...
| `rp`        | `auth:rp:{sha256(oldToken)}`                             | JSON of the **new** session record — never the raw token                                                                          | `refreshGraceWindowSeconds` (default: 30s)                | Rotation pointer (grace window). When the refresh token is rotated, the old token points to the new one for the configured time. This prevents errors in concurrent requests that use the old token.                                                                                                |
...
| `sess`      | `auth:sess:{userId}`                                     | SET whose members are full key suffixes: `rt:{hash}` for a live session, `rp:{oldHash}` for a grace pointer                       | Equal to the max refresh TTL                              | Tracking of the user's active sessions. Each member of the SET is the sha256 of an active refresh token. Used to count and list sessions.                                                                                                                                                           |
...
| `prt`       | `auth:prt:{sha256(token)}`                               | JSON with session data: `{ userId, role, device, ip, createdAt, mfaEnabled, familyId }` (`familyId` omitted when empty)           | `refreshExpiresInDays` (in days, converted to seconds)    | Platform admin refresh token session. Same structure as `rt` but for platform tokens.                                                                                                                                                                                                               |
| `prp`       | `auth:prp:{sha256(oldToken)}`                            | JSON of the **new** platform session record — never the raw token                                                                 | `refreshGraceWindowSeconds` (default: 30s)                | Rotation pointer for platform refresh token. Same logic as `rp` but for the admin flow.                                                                                                                                                                                                             |
...
| `psess`     | `auth:psess:{userId}`                                    | SET whose members are `prt:{hash}` / `prp:{oldHash}` key suffixes                                                                 | Equal to the max refresh TTL                              | Tracking of active sessions of platform admins. Analogous to `sess:{userId}` for the dashboard. Each member is `sha256(refreshToken)` of an active `prt`.                                                                                                                                           |
...
| `cf`        | `auth:cf:{sha256(consumedToken)}`                        | `familyId` of the lineage the consumed token belonged to                                                                          | Equal to the refresh TTL                                  | Consumed-token family marker. Planted in the same atomic step that consumes a token and deliberately outliving the much shorter grace pointer: once the pointer is gone, the marker's presence proves a presented token was legitimately issued and already rotated — a reuse, not a random string. |
| `fam`       | `auth:fam:{familyId}`                                    | SET of **bare** `sha256(refreshToken)` hashes                                                                                     | Equal to the refresh TTL                                  | Refresh-token family index: one login lineage, minted at login and inherited by every rotation. Members are bare hashes (unlike `sess:`) because a family only ever tracks live `rt:` sessions. Deleting the key revokes the lineage and refuses any grace recovery that still names it.            |
| `pcf`       | `auth:pcf:{sha256(consumedToken)}`                       | `familyId`                                                                                                                        | Equal to the refresh TTL                                  | Platform consumed-token family marker. Analogue of `cf`.                                                                                                                                                                                                                                            |
| `pfam`      | `auth:pfam:{familyId}`                                   | SET of bare `sha256(refreshToken)` hashes                                                                                         | Equal to the refresh TTL                                  | Platform refresh-token family index. Analogue of `fam`.                                                                                                                                                                                                                                             |
| `ep`        | `auth:ep:{userId}`                                       | Numeric generation counter (string)                                                                                               | 30 days                                                   | Per-user token epoch. Every access token is stamped with the epoch current at issuance and rejected when stamped below the stored value, so a password reset invalidates every outstanding access token in one write — the stateless counterpart to `rv:`, which revokes one `jti` at a time.       |
