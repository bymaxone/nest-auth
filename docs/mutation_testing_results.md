...
Line coverage proves a line *executed* during a test; mutation testing proves a test *would
fail* if that line were wrong. We seeded thousands of small faults into the source — flipped
...
> hand-authored pass *just before* the re-baseline snapshot, so their re-baseline column already
...
Getting a *trustworthy* run — one whose survivors are real — required five corrections. The first
...
- **GAP** — the suite *should* have caught it. We write a test that fails under the mutation and
...
- **Session-hash format guard.** The `^...$` anchors on the session-hash regex must reject a
  malformed hash *before* any Redis key is constructed from it. Pinned by asserting that Redis is
...
- **Mutually redundant guards.** `value.length === 64 && /^[0-9a-f]{64}$/.test(value)` — each
  conjunct *independently* forces "exactly 64 hex characters", so deleting either still rejects the
  same set of inputs.
...
The final report lists **10 survivors**, and they are all in the *equivalent* set above — but they
