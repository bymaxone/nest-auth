...
describe('SessionService', () => {
  ...
  beforeEach(async () => {
    ...
    const module = await Test.createTestingModule({
      providers: [SessionService, { provide: 'REDIS_CLIENT', useValue: createMockRedis() }]
    }).compile()
...
const config: Config = {
  ...
  // Enforce coverage thresholds — 100% across the board (hard gate)
  coverageThreshold: {
    global: {
      branches: 100,
      functions: 100,
...
100% line coverage means every line _executes_ under test — it does **not** mean a
test would _fail_ if the line regressed. Stryker mutation testing closes that gap by
...
describe('register()', () => {
  ...
  it('should throw ConflictException when email already exists', async () => {
    ...
    // Act & Assert
    await expect(service.register(registerDto, mockUserRepo)).rejects.toThrow(ConflictException)
...
jest
  ...
  .mockReturnValue(Buffer.from('0123456789abcdef0123456789abcdef', 'hex'))

jest.spyOn(crypto, 'scryptSync').mockReturnValue(Buffer.from('mocked-scrypt-output'))
...
type MockRedis = jest.Mocked<
  Pick<
    Redis,
    'get' | 'set' | 'del' | 'expire' | 'exists' | 'keys' | 'ttl' | 'incr' | 'multi' | 'pipeline'
  >
>
...
describe('SessionService', () => {
  ...
  it('should store session data in Redis with TTL', async () => {
    ...
    // Assert
    expect(redis.set).toHaveBeenCalledWith(`session:${sessionId}`, JSON.stringify(sessionData))
...
describe('TOTPService', () => {
  beforeEach(() => {
    ...
    jest
      .spyOn(crypto, 'randomBytes')
      .mockReturnValue(Buffer.from('a1b2c3d4e5f6a7b8a1b2c3d4e5f6a7b8', 'hex') as any)
  })
...
| -------------- | -------- | ---------------------------------------------------------------- |
| **Branches**   | **100%** | All `if/else`, ternary, `switch`, `??`, `?.` paths tested        |
| **Functions**  | **100%** | All exported and private functions invoked during tests          |
| **Lines**      | **100%** | Physical lines of code executed during tests                     |
| **Statements** | **100%** | All statements (may differ from lines for multi-statement lines) |
...
The global threshold is **100% across every layer** — there is no lower tier for
any directory (guards, crypto, controllers, hooks, utils all sit at 100%). 100%
line coverage proves code _executes_; it does NOT prove a test would _fail_ on a
regression, so the deeper gate is **mutation testing** (Stryker `break: 95`).
Treat 100% line coverage as the floor and a high mutation score as the real
measure of test quality, especially on security-critical code (`crypto/`,
`guards/`).
...
- [ ] Running `pnpm test:cov` shows all four metrics at 100% (the enforced gate).
- [ ] Mutation testing (Stryker `break: 95`) passes — the deeper gate beyond 100% line coverage, especially on security-critical modules (guards, password, tokens).
